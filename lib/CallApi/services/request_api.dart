...
class RequestApi {
  ...
  Future<List<AccountModel>> getData() async {
    ...
    if (response.statusCode == 200) {
      ...
      List<AccountModel> listAccount = jsonData.map((e) => AccountModel.fromJson(e)).toList();
      //
      return listAccount;
  ...
  static Future<dynamic> postData() async {
    ...
    var url = Uri.parse('${baseUrl}getDataFromApi.php');
    AccountModel model = AccountModel(userName: 'test api', password: '123456aA', fullName: 'aa bb cc', avatar: 'avatar', type: 'user', status: '1');
    var encodeObject = json.encode(model.toJson());

    var response = await client.post(url, body: encodeObject, headers: {"Content-type":"application/json"});
